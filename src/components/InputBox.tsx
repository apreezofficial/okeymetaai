import { Paperclip, Mic, FileText, ImageIcon, Camera, Brain, Search, Sparkles, ArrowUpRight, Loader2, Settings2 } from 'lucide-react'; import { useRef, useState } from 'react';
...
{/* Voice Input */}
      ...
      <div className="hidden md:flex items-center gap-1 mr-2">
        {utilityOptions.map(({ key, icon, label }) => (
          <button
            ...
            className={`p-2 rounded-lg transition text-gray-500 dark:text-gray-400 hover:text-primary hover:bg-gray-100 dark:hover:bg-gray-800 ${selectedUtilities.includes(key) ? 'bg-gray-200 dark:bg-gray-700 text-primary' : ''}`}
            title={label}
          >
            {icon}
          </button>
      ...
      <div className="block md:hidden relative">
        ...
        {showMobileMenu && (
          <div className="absolute bottom-12 right-0 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 shadow-lg rounded-lg z-20 p-2 flex flex-col gap-2 w-40">
            {utilityOptions.map(({ key, icon, label }) => (
              <button
                key={key}
                onClick={() => toggleUtility(key)}
                className={`flex items-center gap-2 p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 transition ${selectedUtilities.includes(key) ? 'bg-gray-200 dark:bg-gray-700 text-primary' : 'text-gray-600 dark:text-gray-300'}`}
              >
      ...
      <button
        ...
        className={`ml-2 transition-all ${!externalInput.trim() ? 'opacity-50 cursor-not-allowed' : 'hover:scale-105'}`}
